...
func Middleware(writer http.ResponseWriter, request *http.Request) {
	...
	case "/_proxy/fingerprint":
		...
		return
	case "/_proxy/verified":
		writer.Header().Set("Content-Type", "text/plain")
		SendResponse("verified", buffer, writer)
		return
	case "/_proxy/" + proxy.AdminSecret + "/api/v1":
	...
	if strings.HasPrefix(request.URL.Path, "/_proxy/api/v2") {
